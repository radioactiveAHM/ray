...
fn parse_target(buff: &[u8])->Result<(String, usize, SocketType), VError> {
    match buff[21] {
        1 => {
            Ok((std::net::IpAddr::V4(std::net::Ipv4Addr::new(buff[22], buff[23], buff[24], buff[25])).to_string(), 26, parse_socket(buff[18])?))
        },
        2 => {
            if let Ok(s) = core::str::from_utf8(&buff[23..buff[22] as usize + 23]) {
                Ok((s.to_string(), 23 + buff[22] as usize, parse_socket(buff[18])?))
            } else {
                Err(VError::UTF8Err)
            }
        },
